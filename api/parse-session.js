...
async function handler(request,response){
  ...
  try{
    ...
    if(!result.ok){if(result.code==="AI_INVALID_RESPONSE"){const draft=localClarificationDraft(description,clientDate),clarification=clarificationResult(draft);if(clarification.status==="needs_clarification")return send(response,200,{ok:true,draft,...clarification,warnings:[]});}return send(response,result.status,{ok:false,code:result.code});}
    if(!result.draft.date)result.draft.date=clientDate;
